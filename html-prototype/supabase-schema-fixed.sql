-- Dermal Skin Clinic - Corrected Supabase Database Schema
-- Fixed RLS policies and syntax errors for proper Supabase implementation
...
-- Services - Public read access, authenticated users can manage
...
-- Rooms - Public read access, authenticated users can manage
...
-- Staff - Controlled access based on authentication
...
-- Customers - Users can access their own data, staff can access all
...
-- Bookings - Customers see their bookings, authenticated users see all
...
-- Staff schedules - Staff manage their own schedules, others can view
...
-- Service packages - Public read, authenticated users can manage
...
-- Payments - Restricted to authenticated users only
...
('Hot Stone + Microderm', 'Luxury hot stone massage with microdermabrasion facial', 
 ARRAY['hot_stone_massage', 'microderm_facial'], 120, 189, 200, true);
