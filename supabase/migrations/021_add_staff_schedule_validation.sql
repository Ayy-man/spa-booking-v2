-- Add staff schedule validation to prevent booking staff when they're not available
-- This fixes the REAL root cause of "Staff member is not available at the requested time" error
...
COMMENT ON FUNCTION check_staff_availability IS 'Trigger function that validates staff availability before booking creation/update';
