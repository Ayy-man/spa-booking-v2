-- Fix process_couples_booking_v2 function signature to match frontend expectations
-- This addresses the parameter order mismatch causing the booking confirmation error
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT NULL::UUID, NULL::UUID, NULL::INTEGER, false, 'Secondary service not found';
        RETURN;
    END IF;
...
-- Update the Supabase client to use the new function
COMMENT ON FUNCTION process_couples_booking_v2 IS 'Creates couples bookings with corrected parameter order matching frontend expectations';