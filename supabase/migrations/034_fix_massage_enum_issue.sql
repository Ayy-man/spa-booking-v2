-- Fix massage detection without relying on service_category enum
-- Simply remove the staff restriction for massage services based on name/GHL category
...
BEGIN
    ...
    -- Check if both services are massages based on GHL category or name patterns
    -- Avoid using the enum field to prevent type errors
    IF (v_primary_service.ghl_category = 'BODY MASSAGES' OR
        ...
        LOWER(v_primary_service.name) LIKE '%stretching%') 
       AND
    ...
    -- SIMPLIFIED: Only check for same staff if NOT a massage booking
    -- For massages, allow same staff (Robyn can handle both)
    IF p_primary_staff_id = p_secondary_staff_id AND NOT v_is_massage_booking THEN
        RETURN QUERY SELECT NULL::UUID, NULL::INTEGER, NULL::UUID, false, 
            'Same staff cannot be selected for non-massage services. For massage services, the same therapist can handle both guests.';
        RETURN;
    END IF;
...
DO $$
BEGIN
    RAISE NOTICE '✅ MASSAGE BOOKING FIX COMPLETE!';
    RAISE NOTICE '   ';
    RAISE NOTICE '🎯 Fixed Issues:';
    RAISE NOTICE '   - Removed enum type error for service_category';
    RAISE NOTICE '   - Detection based on GHL category and service names only';
    RAISE NOTICE '   - Same staff allowed for massage services';
    RAISE NOTICE '   - Different staff required for non-massage services';
    RAISE NOTICE '   ';
    RAISE NOTICE '✅ Robyn can now handle both clients for couples massages!';
END $$;