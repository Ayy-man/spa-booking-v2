-- Fix the "column 'schedule' does not exist" error by updating validation functions
-- to use the actual database schema: staff.work_days array + staff_schedules table

-- Drop the problematic functions from the previous migration
...
-- Create corrected staff schedule validation function
...
-- Create trigger function that validates staff availability
...
COMMENT ON FUNCTION validate_staff_schedule IS 'Validates if a staff member is available based on work_days array and staff_schedules table for a specific date and time';
COMMENT ON FUNCTION validate_booking_staff_schedule IS 'Trigger function that validates staff availability before booking creation/update using actual database schema';

-- Test cases (these return TRUE/FALSE for validation)
-- These comments show example usage:
-- SELECT validate_staff_schedule('leonel-sidon', '2024-08-04', '10:00', '11:00'); -- Sunday for Leonel (should be TRUE if he works Sundays)
-- SELECT validate_staff_schedule('selma-villaver', '2024-08-06', '10:00', '11:00'); -- Tuesday for Selma (should be FALSE if she doesn't work Tuesdays)