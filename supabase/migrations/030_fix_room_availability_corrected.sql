-- CORRECTED FIX: Room Availability Issues with INTEGER room IDs
-- This migration fixes all schema mismatches using the correct integer room IDs
...
-- 3. Update services with room requirements
...
WHERE name ILIKE '%couples%' OR category = 'package';

-- 4. Fix room assignment function with correct integer room IDs
...
-- 6. Update room capabilities using INTEGER IDs (1, 2, 3)
UPDATE rooms SET capabilities = ARRAY['facial', 'massage', 'waxing', 'body_treatment']
WHERE id = 1; -- Room 1

UPDATE rooms SET capabilities = ARRAY['facial', 'massage', 'waxing', 'body_treatment'] 
WHERE id = 2; -- Room 2

UPDATE rooms SET capabilities = ARRAY['facial', 'massage', 'waxing', 'body_treatment', 'body_scrub']
WHERE id = 3; -- Room 3 (has special equipment for body scrubs)

-- 7. Ensure rooms exist with proper data
INSERT INTO rooms (id, name, capacity, capabilities, is_active)
VALUES 
    (1, 'Room 1', 1, ARRAY['facial', 'massage', 'waxing', 'body_treatment'], true),
    (2, 'Room 2', 2, ARRAY['facial', 'massage', 'waxing', 'body_treatment'], true),  
    (3, 'Room 3', 2, ARRAY['facial', 'massage', 'waxing', 'body_treatment', 'body_scrub'], true)
ON CONFLICT (id) 
DO UPDATE SET 
    name = EXCLUDED.name,
...
-- 8. Create a simple test function to verify the fix
...
-- Comment
COMMENT ON FUNCTION assign_optimal_room IS 'Assigns optimal room based on service requirements and availability - Uses INTEGER room IDs';
