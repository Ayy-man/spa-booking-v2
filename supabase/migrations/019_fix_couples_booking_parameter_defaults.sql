-- Fix process_couples_booking_v2 parameter order with proper default values
-- This fixes the "Staff member is not available at the requested time" error
-- and the PostgreSQL parameter default value requirement
...
BEGIN
    ...
    IF p_booking_date IS NULL THEN
        RETURN QUERY SELECT NULL::UUID, NULL::INTEGER, NULL::UUID, false, 'Booking date is required';
        RETURN;
    END IF;
    
    IF p_start_time IS NULL THEN
        RETURN QUERY SELECT NULL::UUID, NULL::INTEGER, NULL::UUID, false, 'Start time is required';
        RETURN;
    END IF;
...
-- Comment
COMMENT ON FUNCTION process_couples_booking_v2 IS 'Creates couples bookings with corrected parameter order and proper default values';