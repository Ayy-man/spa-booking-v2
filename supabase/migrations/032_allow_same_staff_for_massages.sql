-- Allow same staff for couples massage bookings
-- This addresses the requirement that Robyn (primary massage therapist) can handle both clients simultaneously

-- Drop the existing function to recreate with massage exception
DROP FUNCTION IF EXISTS process_couples_booking_v2 CASCADE;

-- Recreate the function with special handling for massage services
...
BEGIN
    ...
    -- Check if both services are massages (category = 'massage' or name contains 'massage')
    IF (v_primary_service.category = 'massage' OR LOWER(v_primary_service.name) LIKE '%massage%') AND
       (v_secondary_service.category = 'massage' OR LOWER(v_secondary_service.name) LIKE '%massage%') THEN
        v_is_massage_booking := true;
    END IF;
    ...
    IF p_primary_staff_id = p_secondary_staff_id AND NOT v_is_massage_booking THEN
        RETURN QUERY SELECT NULL::UUID, NULL::INTEGER, NULL::UUID, false, 
            'Cannot book the same staff member for both services (except for massage services)';
        RETURN;
    END IF;
...
'Creates couples bookings with time validation. 
Allows same staff member for massage services as they can handle both clients simultaneously.
Other services still require different staff members.';

-- Success message
DO $$
BEGIN
    RAISE NOTICE '✅ COUPLES MASSAGE BOOKING FIX APPLIED!';
    RAISE NOTICE '   - Same staff can now be selected for massage services';
    RAISE NOTICE '   - Robyn can handle both clients for couples massages';
    RAISE NOTICE '   - Other services still require different staff members';
    RAISE NOTICE '   ';
    RAISE NOTICE '🎯 The booking system now properly handles couples massages!';
END $$;