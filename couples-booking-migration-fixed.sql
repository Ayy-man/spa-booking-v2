-- =====================================================
-- COUPLES BOOKING MIGRATION - MATCHES ACTUAL SCHEMA
-- Based on your actual database schema with correct data types
-- =====================================================
...
-- Add explicit couples services if they don't exist
...
-- Fixed assign_optimal_room function using correct data types
...
BEGIN
    ...
    IF service_record.is_couples_service = true THEN
        -- Try Room 3 first (best for couples)
        SELECT id, name INTO assigned_room_id, assigned_room_name 
        FROM rooms 
        ...
        ELSE
            assigned_room_id := NULL;
            assigned_room_name := NULL;
            assignment_reason := 'No couples rooms available (need capacity >= 2)';
            RETURN NEXT;
            RETURN;
        END IF;
    END IF;
...
-- Success message
SELECT 'Couples booking migration completed successfully! Schema matches your actual database.' as result;